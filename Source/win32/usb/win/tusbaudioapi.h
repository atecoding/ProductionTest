...
//
// Returns the version number of the software interface (API) 
// that is exported by the DLL.
// This is the TUSBAUDIO_API_VERSION constant the DLL has been compiled with.
//
TUSBAUDIO_DECL
unsigned int
TUSBAUDIO_CALL
...
// that is exported by the DLL. This should be called by an application before any
// function of the DLL is used. Pass the TUSBAUDIO_API_VERSION_MJ and TUSBAUDIO_API_VERSION_MN 
// constants from this header file in this call.
//
// Returns 1 if the version matches, the calling application is compatible with the DLL.
// Returns 0 if the version does not match, the calling application is incompatible with the DLL.
//
TUSBAUDIO_DECL
int
TUSBAUDIO_CALL
...
// The device is identified by a zero-based index.
// In case of success a handle is returned that has to be used for all calls with this device.
//
TUSBAUDIO_DECL
TUsbAudioStatus
TUSBAUDIO_CALL
...
//
// close a device (counterpart to TUSBAUDIO_OpenDeviceByIndex)
// After this call the device handle is invalid.
//
TUSBAUDIO_DECL
TUsbAudioStatus
TUSBAUDIO_CALL
...
// The sample rate is returned in 1/s.
//
TUSBAUDIO_DECL
TUsbAudioStatus
TUSBAUDIO_CALL
...
//
// set sample rate
// The sample rate is specified in 1/s.
//
TUSBAUDIO_DECL
TUsbAudioStatus
TUSBAUDIO_CALL
...
TUSBAUDIO_GetFirmwareImageSize(
  unsigned int* numberOfBytes,
  unsigned int flags	// 0 or TUSBAUDIO_FLAG_UPLOADED_IMAGE
  );
typedef
TUsbAudioStatus
TUSBAUDIO_CALL
...
//
// return the firmware image that was uploaded from the USB device
//
TUSBAUDIO_DECL
TUsbAudioStatus
TUSBAUDIO_CALL
...
//
// download the current firmware image to the device
// possible in state DfuProcState_Idle only
//
TUSBAUDIO_DECL
TUsbAudioStatus
TUSBAUDIO_CALL
...
//
// upload the current firmware image from the device
// possible in state DfuProcState_Idle only
//
TUSBAUDIO_DECL
TUsbAudioStatus
...
//
// roll the firmware on the device back to the original image installed
// possible in state DfuProcState_Idle only
//
TUSBAUDIO_DECL
TUsbAudioStatus
...
// Needs to be called after TUSBAUDIO_GetDfuStatus reported
// either DfuProcState_Finished or DfuProcState_Failed.
// Sets state back to DfuProcState_Idle. Returns TSTATUS_SUCCESS in this case.
// Aborts the download procedure when called in any other state, returns TSTATUS_ABORTED in this case.
// Frees internal resources, frees the image loaded with TUSBAUDIO_LoadFirmwareImageFromXxx
//
TUSBAUDIO_DECL
TUsbAudioStatus
TUSBAUDIO_CALL
