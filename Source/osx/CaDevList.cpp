...
void CCaDevList::Dump()
{
	...
	Logger::outputDebugString(String::formatted(T("Device list count %d"),_numids));
...
AudioDeviceID CCaDevList::GetCaId(CFNumberRef cfguid)
{
	...
	uid = String::formatted(T("AppleFWAudioEngineGUID:%llu"),guid);
...
{
	DBG_PRINTF((T("AudioDevListListener %d"),inPropertyID));
