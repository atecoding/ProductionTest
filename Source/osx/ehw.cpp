...
OSStatus ehw::CreatePlugin(FileLogger *log)
{
	...
	DBG_PRINTF((T("plugin create - returned %d"),status));
	if (noErr != status)
		return status;
...
int ehw::readcaps()
{
	...
	DBG_PRINTF((T("readcaps %p"),_fwavc));
	...
	if ((0 == status) && (efc_ok == _rsp->h.rval))
	{
		...
		DBG_PRINTF((T("device name %s  num outputs %d"),_caps->BoxTypeName(),_caps->numbusout()));
	...
	{
		DBG_PRINTF((T("AVCCommand failed for readcaps - status 0x%x  EFC status %d"),status,_rsp->h.rval));
	}
...
int ehw::SendEfc(size_t outbytes,size_t inbytes)
{
	...
	if (0 != status)
	{
		Logger::outputDebugString(String::formatted(T("ehw::SendEfc - AVCCommand - bad status %d (0x%x)"),status,status));
	...
	{
		Logger::outputDebugString(String::formatted(T("AVCCommand - bad EFC rval %d"),_rsp->h.rval));
		return 1;
	}
...
int ehw::updatepolledstuff()
{
	...
	if ((efc_ok != _rsp->h.rval) && (efc_flash_busy != _rsp->h.rval))
	{
		Logger::outputDebugString(String::formatted(T("AVCCommand for polled stuff - bad EFC rval %d"),_rsp->h.rval));
		return 1;
	}
...
int ehw::SetCoreAudioSampleRate(int32 samplerate)
{
	...
	switch (status)
	{
		...
		default :
			DBG_PRINTF((T("set clock - AudioDeviceSetProperty returned %d\n"),status));
			rval = 1;
			break;
	}
...
int ehw::getflashstatus(bool &ready)
{
	...
	if (0 != rval)
	{
		DBG_PRINTF((T("getflashstatus failed")));
	}
